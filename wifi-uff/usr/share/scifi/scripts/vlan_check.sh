...
SRVIP_LAN=$(awk -v search=SRVIP_LAN '{if ($1==search) print $2;}' /etc/scifi/scifi.conf) 
SRVIP_CAD=$(awk -v search=SRVIP_CAD '{if ($1==search) print $2;}' /etc/scifi/scifi.conf) 
SRVIP_VIS=$(awk -v search=SRVIP_VIS '{if ($1==search) print $2;}' /etc/scifi/scifi.conf) 
...
pinglan=$(ping -I br-lan -w10 $SRVIP_LAN |  grep loss | awk '{print $4;exit}') 

        ifconfig br-cadastro 192.168.$major_ip.$minor_ip netmask 255.255.128.0 
        ping203=$(ping -I br-cadastro -w10 $SRVIP_CAD |  grep loss | awk '{print $4}') 
        ifconfig br-cadastro 0.0.0.0 

        let majorplus=major_ip+128 
        ifconfig br-visitante 192.168.$majorplus.$minor_ip netmask 255.255.128.0 
        ping204=$(ping -I br-visitante -w10 $SRVIP_VIS |  grep loss | awk '{print $4}') 
        ifconfig br-visitante 0.0.0.0 
...
# verificando comunicacao na br-cadastro (em bridge com wlan0-1) 
# verificando comunicacao na br-visitante (em bridge com wlan0-2) 
